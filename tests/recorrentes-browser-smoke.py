...
try{
 ...
 if(!document.getElementById('rec-shortcut'))throw Error('Atalho ausente');
 ...
 report(true,'Cadastro, visualização, entrada automática, saldo e idempotência');
...
try:
 with tempfile.TemporaryDirectory() as profile:
  ...
  if 'PASS: Cadastro, visualização, entrada automática, saldo e idempotência' not in result.stdout:
   print('SMOKE_FAILED / Chrome rc',result.returncode,file=sys.stderr)
   print(result.stdout[-3000:],file=sys.stderr)
   print(result.stderr[-1500:],file=sys.stderr)
   sys.exit(1)
  print('PASS: Chrome headless — cadastro, entrada automática, saldo, ausência de duplicidade e navegação')
finally:
