...
function scenario({text='{"versao":2,"dados":{"extrato":[]}}',allow=true,readError=false,invalidSchema=false,renderError=false}={}){
 ...
 const handle={name:'dados.json',getFile:async()=>{seen.read++;seen.activation=false;if(readError)throw Error('Falha de leitura');return{text:async()=>text};}};
 ...
 const ctx=vm.createContext({Promise,JSON,SyntaxError,Error,Number,String,Array,console,S,
  ...
  fileWriteAllowed:async()=>{seen.asked++;assert(seen.activation,'Chrome perdeu ativacao antes de solicitar permissao');return allow;},
 ...
 vm.runInContext('let hasUnsavedChanges=false,dataFileHandle=null,knownFileSignature=null,dataRevision=0,isApplyingData=false;'+match[0],ctx);
...
(async()=>{
 ...
 console.log('PASS: autorizacao antes de leitura; abertura, permissao, JSON invalido, validacao, leitura e rollback sem escritas');
